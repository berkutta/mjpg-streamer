...
int mmal_encoder(void)
{
    ...
    format_in->es->video.width = VCOS_ALIGN_UP(mmal_width, 32);
    format_in->es->video.height = VCOS_ALIGN_UP(mmal_height, 16);
    format_in->es->video.crop.width = mmal_width;
    format_in->es->video.crop.height = mmal_height;
    ...
    format_out->es->video.width = mmal_width;
    format_out->es->video.height = mmal_height;
    ...
    encoder->input[0]->buffer_size = (1024 << 10) * 3;
    encoder->output[0]->buffer_num = 6;
    encoder->output[0]->buffer_size = (1024 << 10) * 3;
    ...
    for (;;)
    {
        ...
        while ((buffer = mmal_queue_get(context.queue)) != NULL)
        {
            ...
            put_mmal_frame_out(buffer->data, buffer->length);
...
void *video_mmal_encoder_function(void) {
    while(1) {
        mmal_encoder();
    }
}
